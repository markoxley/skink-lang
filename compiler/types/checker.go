...
func (c *Checker) isAssignable(dst, src Type) bool {
	...
	// Try the same qualified/unqualified keys used for interface duck-typing
	// so cross-module implementations are recognised.
	for _, tm := range tmpl.Methods {
		...
		var moduleName, structName string
		...
		// module.method (e.g. sensor.name)
		if !found && moduleName != "" && c.resolve(moduleName+"."+methodName) != nil {
			found = true
		}
		// Type.method (unqualified struct name)
		if !found && structName != "" && c.resolve(structName+"."+methodName) != nil {
			found = true
		}
		// bare method name
		if !found && c.resolve(methodName) != nil {
...
func (c *Checker) checkFnLiteral(e *ast.FnLiteral) Type {
	...
	fnType := &FunctionType{Params: params, Ret: []Type{ret}}
	oldFn := c.currentFn
	c.currentFn = fnType
	c.pushScope()
	for i, p := range e.Params {
